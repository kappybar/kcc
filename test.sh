...
assert 4 "int main() {long x; x = 4; return x; } "
assert 0 "long x; int main() {return x; } "
assert 7 "long x = 7; int main() {return x; } "
assert 6 "long add(long x, long y) { return x + y; } int main() {long x = add(1,2); long y = add(x,3); return y; }"
assert 4 "int main() { long x, *y = &x; *y = 4; return x; }"
assert 10 "int main() { long arr[10]; int i;for (i = 0;i < 10; i=i+1) arr[i] = 1;long sum = 0;for (i = 0;i < 10;i = i+1) sum = sum + arr[i]; return sum;  }"
assert 4 "char xc = 1;short xs = 1;int xi = 1;long xl = 1; int main() { return xc + xs + xi + xl; }"
